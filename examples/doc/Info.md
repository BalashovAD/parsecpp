...
- Замена Regexp при определенном формате данных: В случаях, когда формат данных четко определен, Parsecpp может заменить использование Regexp, поскольку парсер не умеет откатывать конечный автомат, как это делает Regexp. Это позволяет легко создать парсер, который может работать в режиме possessive mode.
...
**Note**: Пока не asccii не поддерживается.
...
#### `LazyForget`
Однако даже с `lazyCached` мы должны использовать type erasure через `toCommonType`, 
который использует в реализации `std::function`. 
Вместо этого можно использовать более ручной способ стирания типа хранимой функции. 
Однако, поскольку разница в производительности довольно незначительна, а способ записи менее удобен, обычно нет большого смысла использовать `lazyForget`. В большинстве случаев, использование `toCommonType` с `lazyCached` будет достаточно оптимальным и удобным решением для работы с рекурсивными парсерами.
...
Рассмотрим cтандартное определение грамматики математического выражения c левой рекурсией:
```
E -> E Op E | F
F -> NUMBER | ( E )
Op -> + | - | * | /
...
с) Рекурсия может быть реализована через 3 метода: `lazy`, `lazyCached`, `lazyForget`. 
...
`lazyForget` использует более сложный способ стирания типа, что делает его немного быстрее, чем `lazyCached`. 
...
BM_bracesSuccess/bracesCachedDrop_median         312 ns // Drop
...
BM_bracesFailure/bracesCachedDropF_median        197 ns // Drop
